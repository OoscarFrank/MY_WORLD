...
void move_event(sfEvent event, sfRenderWindow *window, maps *m)
{
...
void launch_event(sfEvent event, sfRenderWindow *window, maps *m, cursor *c)
{
    ...
    if (event.type == sfEvtKeyPressed) {
        if (event.key.code == sfKeyEscape)
            sfRenderWindow_close(window);
        move_event(event, window, m);
    }
    mouse_event(event, window, m, c);
